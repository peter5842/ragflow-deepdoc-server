...
class Pdf(PdfParser):
    ...
    def __call__(self, filename, binary=None, from_page=0,
        ...
        callback(0.6, msg="OCR finished ({:.2f}s)".format(timer() - start))
...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    if re.search(r"\.docx$", filename, re.IGNORECASE):
        ...
        if parser_config.get("html4excel"):
            sections = [(_, "") for _ in excel_parser.html(binary, 256) if _]
        else:
...
def chunk_service(file_url=None, upload_file=None):
    # file_url: 网络文件url
    # upload_file: fastapi UploadFile
    if file_url:
        filename = file_url.split("/")[-1]
        binary = requests.get(file_url, timeout=60).content
    elif upload_file:
        filename = upload_file.filename
        binary = upload_file.file.read()
    else:
    ...
    <html>
        ...
        <body>
            ...
            <form action="/process_url" method="post">
                文件URL: <input type="text" name="file_url" style="width:400px"/>
...
async def process_url(file_url: str = Form(...)):
    try:
        result = chunk_service(file_url=file_url)
        html = "<h3>处理结果：</h3><ul>"
        for item in result:
            if isinstance(item, dict) and "content_with_weight" in item:
                html += f"<li>{item['content_with_weight']}</li>"
            elif isinstance(item, dict) and "error" in item:
                html += f"<li style='color:red'>{item['error']}</li>"
        html += "</ul><a href='/'>返回</a>"
        return html
    except Exception as e:
        return f"<b>处理失败：</b> {e} <br><a href='/'>返回</a>"


@app.post("/upload", response_class=HTMLResponse)
async def upload(file: UploadFile = File(...)):
    try:
        result = chunk_service(upload_file=file)
        html = "<h3>处理结果：</h3><ul>"
        for item in result:
            if isinstance(item, dict) and "content_with_weight" in item:
                html += f"<li>{item['content_with_weight']}</li>"
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("deepdoc_server:app", host="0.0.0.0", port=49521, reload=True)